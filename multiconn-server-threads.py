# multiconn-server-threads.py
import socket
import sys
import types
from _thread import *
import threading

clients = []
clients_name =[]


def broadcast_message(sender_socket, message, server_msg=False):
    if not server_msg:
        message = f"{clients_name[clients.index(sender_socket)]}: {message}"
    for client in clients:
        if client != sender_socket:  # Don't send the message to the sender itself
            try:
                client.send(message.encode('utf-8'))
            except:
                # Handle broken connections by removing the client
                clients_name.remove(clients_name[clients.index(client)])
                clients.remove(client)
                client.close()

def threaded_client(client_socket):
    name_data = c.recv(1024)
    clients_name.append(name_data.decode('utf-8'))
    broadcast_message(c, f"[SERVER] {clients_name[-1]} has joined the chat.", True)
    while True:
        try:
            data = client_socket.recv(1024)  # Receive data from the client
            if data:
                print(f"[CLIENT] {data.decode('utf-8')}")
                broadcast_message(client_socket, data.decode('utf-8'))  # Broadcast the message to all clients
            else:
                print("[DISCONNECTED] Client disconnected.")
                break
        except:
            print("[ERROR] Issue with client connection.")
            break
    clients_name.remove(clients_name[clients.index(client)])
    clients.remove(client_socket)
    client_socket.close()

host, port = sys.argv[1], int(sys.argv[2])
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    s.bind((host, port))
    s.listen()
    print("[SERVER STARTED] Waiting for connections...")
    try:
        while True:
            c, addr = s.accept()
            clients.append(c)
            # client sends name
            print(f"[CONNECTION] Connected to {addr}")
            start_new_thread(threaded_client, (c,))
    except KeyboardInterrupt:
        print("Caught keyboardinterrupt, exiting")

